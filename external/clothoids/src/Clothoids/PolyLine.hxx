...
namespace fmt {
template <>
struct formatter<G2lib::PolyLine> : ostream_formatter {};
}  // namespace fmt
...
class PolyLine : public BaseCurve {
 ...
 private:
  ...
  void resetLastInterval() {
#ifdef CLOTHOIDS_USE_THREADS
    bool ok;
    integer& lastInterval =
        *m_lastInterval.search(std::this_thread::get_id(), ok);
#else
 ...
 public:
  // explicit
  PolyLine() { this->resetLastInterval(); }
  ...
  integer num_segments() const { return integer(m_polylineList.size()); }

  integer numPoints() const { return integer(m_s0.size()); }
  ...
  void build(real_type const* x, real_type const* y, integer npts);
  ...
  void bbox(real_type& xmin, real_type& ymin, real_type& xmax,
            real_type& ymax) const override;

  void bbox_ISO(real_type /* offs */, real_type& /* xmin */,
                real_type& /* ymin */, real_type& /* xmax */,
                real_type& /* ymax */
  ) const override {
  ...
  void bbTriangles(vector<Triangle2D>& tvec,
                   real_type max_angle = Utils::m_pi / 6,  // 30 degree
                   real_type max_size = 1e100,
                   integer icurve = 0) const override;

  void bbTriangles_ISO(real_type offs, vector<Triangle2D>& tvec,
                       real_type max_angle = Utils::m_pi / 6,  // 30 degree
                       real_type max_size = 1e100,
                       integer icurve = 0) const override;

  void bbTriangles_SAE(real_type offs, vector<Triangle2D>& tvec,
                       real_type max_angle = Utils::m_pi / 6,  // 30 degree
                       real_type max_size = 1e100,
                       integer icurve = 0) const override {
  ...
  void eval_ISO(real_type s, real_type offs, real_type& x,
  ...
  void eval_ISO_D(real_type s, real_type offs, real_type& x_D,
                  real_type& y_D) const override {
    integer idx = this->find_at_s(s);
    real_type ss = m_s0[idx];
    m_polylineList[size_t(idx)].eval_ISO_D(s - ss, offs, x_D, y_D);
  }

  void eval_ISO_DD(real_type, real_type, real_type& x_DD,
                   real_type& y_DD) const override {
    x_DD = y_DD = 0;
  }

  void eval_ISO_DDD(real_type, real_type, real_type& x_DDD,
                    real_type& y_DDD) const override {
    x_DDD = y_DDD = 0;
  }
  ...
  //!
  //! Compute the point at minimum distance from a point `[x,y]` and the line
  //! segment
  //!
  ...
  //!
  integer closest_point_ISO(real_type x, real_type y, real_type& X,
                            real_type& Y, real_type& S, real_type& T,
                            real_type& DST) const override;

  integer closest_point_ISO(real_type /* x    */, real_type /* y    */,
                            real_type /* offs */, real_type& /* X    */,
                            real_type& /* Y    */, real_type& /* S    */,
                            real_type& /* T    */, real_type& /* DST  */
  ) const override {
  ...
  bool collision_ISO(real_type offs, PolyLine const& CL,
  ...
  bool collision_ISO(real_type offs, BaseCurve const* pC,
                     real_type offs_C) const override;
  ...
  //!
  void intersect(PolyLine const& pl, vector<real_type>& ss0,
  ...
  //! \param[out] ilist   list of the intersection (as parameter on the curves)
  //!
  void intersect_ISO(real_type offs, PolyLine const& pl, real_type offs_pl,
  ...
  string info() const { return fmt::format("PolyLine\n{}\n", *this); }
