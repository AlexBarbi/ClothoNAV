...
class GenericThreadPool {
 ...
 public:
  ...
  void rethrow_exception() {
    if (m_pending_exception && !std::uncaught_exceptions()) {
      m_queue->shutdown();
      join_workers();
   ...
   */
  void ignore_thread_pool_exceptions(bool flg = true) {
...
class ThreadPool {
 ...
 public:
  ...
  void resize(unsigned thread_count = std::thread::hardware_concurrency(),
              unsigned queue_size = 0, unsigned maxpart = 0) {
    this->~ThreadPool();
    if (queue_size == 0) queue_size = 50 * (thread_count + 1);
    if (maxpart == 0) maxpart = 3 * (thread_count + 1);
