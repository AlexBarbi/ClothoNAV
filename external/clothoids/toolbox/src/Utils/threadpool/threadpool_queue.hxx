...
class ForEach_Queue {
 ...
 public:
  ForEach_Queue(Iterator& first, Last const& last, Function& fun,
                unsigned = 0 /*ignored*/
                )
      : m_current(first), m_last(last), m_fun(fun) {}

  void work(bool /*ignored*/) {
...
class ForEach_Queue<Iterator, Last, Function, true>
    : public ForEach_Queue<Iterator, Last, Function, false> {
...
class Transform_Queue {
  struct Results {
    typename std::remove_reference<decltype(std::declval<Function&>()(
        *std::declval<InputIterator>()))>::type result;
 ...
 public:
  Transform_Queue(InputIterator& first, Last const& last,
                  OutputIterator& result, Function& fun, unsigned)
      : m_current(first), m_last(last), m_result(result), m_fun(fun) {}

  void work(bool return_if_idle) {
        ...
        */
        ...
        if (m_output_counter == ctr) {
          ...
          *m_result = std::move(results->result);
          ...
          if (m_output_queue_waiters)
            m_output_queue_cond.notify_all();  // All because we do not know who
                                               // is the right one.
        } else {
...
class Transform_Queue<InputIterator, Last, OutputIterator, Function, true> {
 ...
 public:
  Transform_Queue(InputIterator& first, Last const& last,
                  OutputIterator& result, Function& fun, unsigned maxpart)
      : m_current(first),
        m_last(last),
        m_result(result),
        m_fun(fun),
...
class HQueue {
  ...
  */
  class Queue {
   ...
   public:
    Queue(unsigned s) : m_fun_vec(std::size_t(s + 1)), m_size(s + 1) {}
  ...
  */
      ...
      while ((queue_size = m_queue.size()) <= min_queue_size) {
        ...
        lock.unlock();
        {
          ...
          while (m_queue.empty() && !m_shutting_down) {
            if (++m_idle_workers == m_total_workers)
   ...
   */
  void try_help(int return_if_idle) {
    try {
      help(return_if_idle);
    } catch (...) {
      shutdown();
 ...
 public:
   ...
   */
  void work(bool return_if_idle) { help(return_if_idle ? 0 : -1); }
  ...
  */
  ...
  void wait() {
    if (std::uncaught_exceptions()) shutdown();
    std::exception_ptr e;
    std::unique_lock<std::mutex> lock(m_push_mutex);
    while (!m_queue.empty() || m_idle_workers != m_total_workers) {
      while (!m_queue.empty()) {
        ...
        try {
          ...
          if (e == nullptr) e = std::current_exceptions();
        }
        lock.lock();
      }
