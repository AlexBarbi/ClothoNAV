...
class HQueue {
  ...
  void help(int return_if_idle) {
    ...
    for (;;) {
      ...
      if (stride > function_queue.capacity())
        function_queue.reserve(2 * stride);
  ...
  */
  ...
  void wait() {
    ...
    if (e != nullptr && !std::uncaught_exceptions())
...
class GenericThreadPool {
 ...
 public:
  ...
  GenericThreadPool(Queue* queue, int thread_count)
      : m_queue(queue), m_worker_threads(thread_count) {
  ...
  ~GenericThreadPool() {
    ...
    if (std::uncaught_exceptions()) m_queue->shutdown();
    join();  // Running threads would continue to access the destructed pool.
  }

  unsigned thread_count() const { return unsigned(m_worker_threads.size()); }
};
...
class QueueElement {
 ...
 public:
  ...
  QueueElement(QueueElement&& x) noexcept : m_task(x.m_task) {
    x.m_task = nullptr;
  }
  void operator()() {
    (*m_task)();
...
class ThreadPool2 : public ThreadPoolBase {
 ...
 public:
  explicit ThreadPool2(
      ...
      unsigned queue_size = 0, unsigned maxpart = 0) {
  ...
  void run_task(std::unique_ptr<VirtualTask>&& t) { m_queue->put(t.release()); }

  void run_task(VirtualTask* t) { m_queue->put(t); }

  void exec(std::function<void()>&& fun) override {
    class WrappedFunction : public VirtualTask {
      std::function<void()> m_f;

     public:
      explicit WrappedFunction(std::function<void()>&& f) : m_f(std::move(f)) {}
  ...
  void resize(unsigned thread_count) override {
